            ...
            else
            {
              ...
              if (config.baseline.one_sided_rdma_enabled)
              {
                ...
                if (remote_machine_index_to_rdma != base_index)
                {
                  if (config.baseline.one_sided_rdma_enabled && config.baseline.use_cache_indexing)
                  {
                    found_in_rdma = rdma_node.rdma_key_value_cache->read_callback(key_index, [&, base_index, num_servers, server_start_index, remote_index, remote_port, expected_key=key_index](const RDMACacheIndexKeyValue& kv)
                    {
