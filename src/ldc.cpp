...
// struct Connection
// {
//   Connection(BlockCacheConfig config_, int local_machine_index_, int remote_machine_index_) : config(config_), local_machine_index_(local_machine_index), remote_machine_index(remote_machine_index_)
//   {
...
struct Connection
{
  Connection(BlockCacheConfig config_, int machine_index_, int thread_index_) : config(config_), machine_index(machine_index_), thread_index(thread_index_)
  {
  ...
  void connect_to_remote_machine(int remote_index)
  {
    ...
    ret = machnet_connect(channel, ip.c_str(), remote_machine_config.ip.c_str(), port, &flow);
  ...
  void poll_receive(int index, auto&& handler)
  {
    bool received_data = false;
    while (!received_data)
    {
      received_data = receive(index, handler);
      if (g_stop)
      {
        break;
      }
  ...
  bool receive(int index, auto&& handler)
  {
    std::array<char, 1024> buf;
    auto& connection_data = machine_index_to_connection[index];
    auto& [channel, flow] = connection_data;
    auto ret = machnet_recv(channel, buf.data(), buf.size(), &flow);
    ...
    for (Packet::Reader packet : packets.getPackets())
    {
      ...
      // MachnetFlow tx_flow;
      // tx_flow.dst_ip = flow.src_ip;
      // tx_flow.src_ip = flow.dst_ip;
      // tx_flow.dst_port = flow.src_port;
      // tx_flow.src_port = flow.dst_port;
      handler(data);
    }
    return true;
  }
...
{
  Client(BlockCacheConfig config, int machine_index, int thread_index) : Connection(config, machine_index, thread_index)
  {
...
{
  Server(BlockCacheConfig config, int machine_index, int thread_index) : Connection(config, machine_index, thread_index)
  {
    listen();
  }
  ...
  void get_response(int index, ResponseType response_type, std::string_view value)
  {
    ::capnp::MallocMessageBuilder message;
    Packets::Builder packets = message.initRoot<Packets>();
    ::capnp::List<Packet>::Builder packet = packets.initPackets(1);
...
void client_worker(BlockCacheConfig config, int machine_index, int thread_index) {
...
void server_worker(BlockCacheConfig config, int machine_index, int thread_index, std::shared_ptr<BlockCache<std::string, std::string>> block_cache) {
  ...
  {
    // TODO: remove index
    server.poll_receive(machine_index, [&](auto&& data) {
      if (data.isPutRequest())
      {
        auto p = data.getPutRequest();
        printf("Received put request: key = %s, value = %s\n", p.getKey().cStr(), p.getValue().cStr());
        ...
        server.put_response(index, ResponseType::OK);
      }
      else if (data.isGetRequest())
      {
        auto p = data.getGetRequest();
        server.get_response(index, ResponseType::OK, block_cache->get(p.getKey().cStr()));
      }
    });
  }
}
